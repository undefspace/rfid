...
uint8_t pn532_mifareclassic_AuthenticateBlock(pn532_t *obj, uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    ...
    uint8_t uid_offs = (obj->_uidLen == 7) ? 3 : 0;
    for (i = 0; i < 4; i++)
    {
        pn532_packetbuffer[10 + i] = obj->_uid[uid_offs + i]; /* 4 uint8_t card ID */
    }

    if (!pn532_sendCommandCheckAck(obj, pn532_packetbuffer, 14, 1000))
        return 0;
